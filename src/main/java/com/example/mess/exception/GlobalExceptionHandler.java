...
public class GlobalExceptionHandler {
    ...
    /*
     * =========================================================================
     * 【ADR-010】异常分层与HTTP映射策略
     * =========================================================================
     * 上下文：应用中存在多种异常（参数校验、资源不存在、唯一约束冲突、未知异常），
     *         需要统一映射到 HTTP 状态码和 ApiResponse 格式。
     * 决策：自定义业务异常按语义映射（404/409/400），未知异常兜底映射 500。
     *       全局异常处理器集中处理，Controller 不写 try-catch。
     * 替代方案：
     *   A) Controller 里 try-catch 返回错误 JSON —— 重复代码多，容易遗漏。
     *   B) 所有异常都返回 200 + 业务码 —— 监控/网关无法识别真实故障。
     *   C) 不处理，让 Spring 默认返回 Whitelabel Error Page —— 前端无法解析。
     * 后果：所有异常统一格式返回前端；5xx 不透出堆栈；4xx 记 WARN，5xx 记 ERROR；
     *       但 Security 过滤器链中的异常（401/403）不经过此处理器，需单独配置。
     *
     * =========================================================================
     * 【代码审查要点】异常处理
     * =========================================================================
     ...
     * [ ] 响应体含 path 字段，便于日志关联
     * [ ] Controller 不写 try-catch 返回错误 JSON（由全局处理器兜底）
     * [ ] Security 的 401/403 需单独配置 AuthenticationEntryPoint/AccessDeniedHandler
     * [ ] 异常 message 面向用户可读，不含技术细节
     * =========================================================================
     */

    /*
     * =========================================================================
     * 【ADR-010-S】异常分类与日志级别策略（补充）
     * =========================================================================
     * 上下文：不同类型的异常需要不同的日志级别和处理方式，
     *         统一记 ERROR 会导致噪音，统一记 INFO 会漏掉关键问题。
     * 决策：4xx 记 WARN（客户端问题，不是 bug），5xx 记 ERROR（服务端 bug），
     *       预期内业务异常（如 404）记 WARN 或 INFO。
     * 替代方案：
     *   A) 全部记 ERROR —— 噪音大，4xx 不是 bug 不应告警。
     *   B) 全部记 INFO —— 5xx 会被淹没，无法触发告警。
     *   C) 不记日志 —— 排障时无迹可寻。
     * 后果：监控告警只看 5xx 速率上升；4xx 速率上升关注但不告警；
     *       日志中能区分"用户用错了"和"系统有 bug"。
     *
     * 异常分类决策树：
     *   客户端参数有问题？ -> 4xx（改了请求再试）
     *   资源不存在？ -> 404（不是错误，是"没有"）
     *   权限不够？ -> 403（不泄露"存在但无权"）
     *   服务端代码 bug 或依赖故障？ -> 5xx（可告警，可有限重试）
     *
     * 不吞异常原则：
