...
public class HelloController {
    ...
    /*
     * ============================================================================
     * 【阅读笔记】HelloController：轻量级示例端点（非可执行代码）
     * ============================================================================
     *
     * 一、这类"Hello"端点的真实用途
     * ----------------------------------------------------------------------------
     *   1) 联调前置——验证服务能通、认证能通过、序列化无异常;
     *   2) 灰度验证——灰度发布时先给 1% 流量跑 hello，稳定后再上业务;
     *   3) 框架验收——从请求到响应的最简路径，用于验证框架集成。
     *
     * 二、@Value 与配置注入
     * ----------------------------------------------------------------------------
     *   @Value("${app.greeting:Hello}")
     *       —— 占位符 `${}` 读取配置项，`:Hello` 是兜底值，无配置时也能启动;
     *   注意：默认注入是启动时一次性完成的，配置热更新需配合 @RefreshScope。
     *
     * 三、SLF4J 日志使用要点
     * ----------------------------------------------------------------------------
     *   - 使用占位符 `{}` 拼接，避免 `log.info("x" + name)`，后者在 debug 级别
     *     也会产生字符串拼接的开销;
     *   - 高频请求中避免 large log，避免淹没真正的问题线索;
     *   - 结构化日志推荐 JSON 格式（如 logback-json），便于接入日志平台。
     *
     * 四、有别于 Actuator 健康探针
     * ----------------------------------------------------------------------------
     *   /hello 属于"业务接口"，会进入应用本身的过滤器链、异常处理器;
     *   /actuator/health 是框架级探针，不走业务逻辑。两者不要混用。
     * ============================================================================
     */
}