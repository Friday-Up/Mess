...
public class User {
    ...
    /*
     * ============================================================================
     * 【阅读笔记】JPA 注解词典（本类用到的注解逐个说明，非可执行代码）
     * ============================================================================
     *
     * 一、类级注解
     * ----------------------------------------------------------------------------
     *   @Entity
     *       —— 声明本类是被 JPA 管理的实体。注意：实体类不能是 final，
     *          且需要一个无参构造器（JPA 反射创建对象时使用）。
     *   @Table(name = "users")
     *       —— 指定映射的数据库表名。不写则默认用类名首字母小写（user）。
     *          用 "users" 往往是规避 user 在部分数据库中的保留字冲突。
     *
     * 二、主键相关
     * ----------------------------------------------------------------------------
     *   @Id                —— 标识主键字段;
     *   @GeneratedValue(strategy = GenerationType.IDENTITY)
     *       —— 主键由数据库自增列生成，适合 MySQL。
     *          对比：SEQUENCE（Oracle/PostgreSQL 序列）、
     *                TABLE（用表模拟序列，性能差，少用）、
     *                AUTO（由方言决定）。
     *   @Column(name = "username", nullable = false, unique = true, length = 50)
     *       —— 列名、非空、唯一、长度。注意：unique 在 JPA 自动建表时生效，
     *          但生产环境的表通常由 DDL 脚本管理，实体注解与真实库结构
     *          不一致时不会报错，务必以数据库约束为准。
     *
     * 三、时间字段
     * ----------------------------------------------------------------------------
     *   LocalDateTime 是 Java 8 时间 API，与数据库 datetime 映射良好。
     *   @CreationTimestamp / @UpdateTimestamp（Hibernate 提供）可自动维护，
     *   也可用 @PrePersist / @PreUpdate 回调或 JPA Auditing 实现。
     *
     * 四、迁移提示
     * ----------------------------------------------------------------------------
     *   改用 Lombok 可省去 getter/setter，但要注意 @Data 生成的 equals/hashCode
     *   对 JPA 实体的影响：可变字段参与 hashCode 会破坏基于实体 id 的集合行为。
     * ============================================================================
     */
}